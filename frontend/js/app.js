document.addEventListener('DOMContentLoaded', () => {
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (data.type === 'SCANNER_UPDATE') {
          ...
          // If current selected stock in update, refresh details
  ...
  // Form Handlers inside Modal
  const fyersForm = document.getElementById('fyers-config-form');
  if (fyersForm) {
    fyersForm.addEventListener('submit', async (e) => {
      e.preventDefault();
      const appId = document.getElementById('fyers-app-id').value;
      const token = document.getElementById('fyers-access-token').value;
      await settingsModal.saveFyersCredentials(appId, token);
      await settingsModal.switchAdapter('fyers');
      alert('Fyers credentials saved and connected!');
      settingsModal.close();
    });
  }

  const kiteForm = document.getElementById('kite-config-form');
  if (kiteForm) {
    kiteForm.addEventListener('submit', async (e) => {
      e.preventDefault();
      const apiKey = document.getElementById('kite-api-key').value;
      const token = document.getElementById('kite-access-token').value;
      await settingsModal.saveKiteCredentials(apiKey, token);
      await settingsModal.switchAdapter('kite');
      alert('Kite credentials saved and connected!');
      settingsModal.close();
    });
  }
  ...
  if (switchSimBtn) {
    switchSimBtn.addEventListener('click', async () => {
      ...
      alert('Switched to Live Market Simulator!');
      settingsModal.close();
    });
  }
